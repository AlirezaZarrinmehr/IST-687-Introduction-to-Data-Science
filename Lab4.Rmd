...
```{r}
undergard_sample <- sample(undergrads, size=10, replace=FALSE)
undergard_sample
```
...
```{r}
hist(replicate(3,mean(sample(undergrads, size=10, replace=FALSE))))
```
...
```{r}
means<-replicate(100,mean(sample(undergrads, size=10, replace=FALSE)))
hist(means)
```
...
```{r}
#we are somehow confident that the test sample does not correspond to undergrad because the mean does not fall between the quantile values
...
```{r}
gard_sample <- sample(grads, size=10, replace=FALSE)
gard_sample

mean(sample(grads, size=10, replace=FALSE))

replicate(3,mean(sample(grads, size=10, replace=FALSE)))

hist(replicate(3,mean(sample(grads, size=10, replace=FALSE))))

meansgrad<-replicate(100,mean(sample(grads, size=10, replace=FALSE)))
hist(means)

quant <- quantile(meansgrad, probs = c (0.025, 0.975))
quant
mean(testSample)
if((mean(testSample)< quant[1])||(mean(testSample)> quant[2])){
  print("mean was extreme")
