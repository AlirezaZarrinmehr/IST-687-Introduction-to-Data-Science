...
```{r}
myGrocIF <- itemFrequency(myGroc)
str(myGrocIF)
? sort
myGrocIF <- sort(-myGrocIF)
head(myGrocIF,14)
tail(myGrocIF,14)
```
...
itemFrequencyPlot(myGroc, topN=100)
? itemFrequencyPlot
```
...
ct[1:14,	1:14]
#For example, when whole milk is present, other vegetable are present 736 times. the matrix is a diagonal matrix. 
```
...
rules2 <- apriori(myGroc,
 parameter=list(supp=0.0006, conf=0.45),
 control=list(verbose=F),
 appearance=list(default="lhs",rhs=("bottled beer")))
inspect(rules2)
#inspectDT(rules2)
#yes, we see more rules now
```
...
Mycars1 <- mtcars
view(Mycars1)
Mycars1$bad_mpg <- as.factor(Mycars1$mpg <=25)
str(Mycars1)
Mycars1 <- Mycars1 %>% mutate_if (is.numeric, as.factor)
str(Mycars1)
#Converting the data frame to a transaction data set (matrix)
mycarstransactions <- as(Mycars1, 'transactions')
#Predicting the rules for having baf mpg. using arules model
rules1 <- apriori(mycarstransactions,
 parameter=list(supp=0.3, conf=0.55),
 control=list(verbose=F),
 appearance=list(default="lhs",rhs=("bad_mpg=TRUE")))
#Create and view rules
inspect(rules1)
#inspectDT(rules1)
```
